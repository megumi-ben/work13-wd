...
# -*- coding: utf-8 -*-
"""
Regex -> Prefix 训练代码（动态数据生成版本 V2）

新增特性：
1. 样本权重 (sample_weight) - 支持基于 expose_ratio 或自定义的权重
2. 学习率调度 (OneCycleLR) - 与静态版一致
3. 梯度累积 (gradient accumulation) - 支持大batch训练

使用示例：
    python train_regex2prefix_dynamic_v2.py \
...
def generate_regex_from_seed(
    ...
    返回：
        (regex, actual_expose_ratio)
        actual_expose_ratio 可用于计算样本权重
    """
    if not tokens:
        return None, 0.0
    ...
    # 计算实际暴露比例
    actual_expose_ratio = exposure_sum / W if W > 0 else 0.0
    ...
    return "".join(parts), actual_expose_ratio
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in tqdm(f, desc="Loading seed strings"):
            ...
            try:
                ...
                # 保留原始记录中的额外信息（可能包含权重相关字段）
                extra = {k: v for k, v in obj.items() if k != seed_field}
...
def compute_sample_weight(
    rec: Dict[str, Any],
    actual_expose_ratio: float,
    prefix_len: int,
    weight_mode: str = "expose",
    expose_alpha: float = 0.5,
    prefix_alpha: float = 0.0,
    tb_beta: float = 0.0,
    ml_beta: float = 0.0,
) -> float:
    """
    ...
    Args:
        rec: 原始记录（可能包含 stats 字段）
        actual_expose_ratio: 实际暴露比例
        prefix_len: prefix 长度
        weight_mode: 权重计算模式
            - "uniform": 均匀权重 (1.0)
            - "expose": 基于暴露比例 (越低权重越高)
            - "prefix": 基于 prefix 长度 (越长权重越高)
            - "combined": 综合多种因素
        expose_alpha: 暴露比例权重指数
        prefix_alpha: prefix 长度权重指数
        tb_beta: trgm_budget 权重指数（静态版兼容）
        ml_beta: max_literal_run_len 权重指数（静态版兼容）
    ...
    if weight_mode in ("expose", "combined"):
        # 暴露比例越低，样本越难，权重越高
        if expose_alpha > 0 and actual_expose_ratio > 0:
            w *= 1.0 / ((actual_expose_ratio + 0.01) ** expose_alpha)
    
    if weight_mode in ("prefix", "combined"):
        # prefix 越长，权重越高
        if prefix_alpha > 0 and prefix_len > 0:
            w *= (prefix_len ** prefix_alpha)
...
class DynamicDataScheduler:
    ...
    def __init__(
        ...
        self.expose_alpha = expose_alpha
        self.prefix_alpha = prefix_alpha
        self.tb_beta = tb_beta
        self.ml_beta = ml_beta
        ...
        print(f"[Scheduler] Weight mode: {weight_mode}, expose_alpha={expose_alpha}")
    ...
    def get_epoch_data(self, epoch: int) -> Tuple[List[str], List[str], List[float]]:
        """
        获取指定 epoch 的训练数据
        ...
        for idx in chunk_indices:
            ...
            # 动态生成 regex（返回实际暴露比例）
            regex, actual_expose = generate_regex_from_seed(
                seed=seed,
                tokens=tokens,
                rng=epoch_rng,
                n_gram=self.n_gram,
            ...
            weight = compute_sample_weight(
                rec=rec,
                actual_expose_ratio=actual_expose,
                prefix_len=len(prefix),
                weight_mode=self.weight_mode,
                expose_alpha=self.expose_alpha,
                prefix_alpha=self.prefix_alpha,
                tb_beta=self.tb_beta,
                ml_beta=self.ml_beta,
            )
        ...
        print(f"[Epoch {epoch}] Round {round_idx+1}, Chunk {chunk_idx+1}/{self.n_chunks}, "
              f"Generated {len(src_list)} pairs, avg_weight={sum(weight_list)/max(1,len(weight_list)):.4f}")
        
        return src_list, trg_list, weight_list
...
def build_eval_data(
    ...
    for idx in indices:
        ...
        # 生成固定的 regex
        regex, _ = generate_regex_from_seed(
            seed=seed_str,
            tokens=tokens,
...
class Regex2PrefixDataset(Dataset):
    ...
    def __init__(
        ...
        self.sample_weights = sample_weights if sample_weights else [1.0] * len(src_list)
        
        assert len(src_list) == len(trg_list) == len(self.sample_weights)
    ...
    def __getitem__(self, idx):
        ...
        # token 级别权重（全 1）
        trg_weights = [1.0] * len(trg_idx)
        
        return (
            torch.tensor(src_idx, dtype=torch.long),
            torch.tensor(trg_idx, dtype=torch.long),
            torch.tensor(trg_weights, dtype=torch.float32),
...
def make_collate_fn(pad_idx: int):
    ...
    def collate_fn(batch):
        src_batch, trg_batch, trg_weight_batch, sample_weight_batch = zip(*batch)
        return (
            pad_sequence(src_batch, batch_first=True, padding_value=pad_idx),
            pad_sequence(trg_batch, batch_first=True, padding_value=pad_idx),
            pad_sequence(trg_weight_batch, batch_first=True, padding_value=1.0),
            torch.stack(sample_weight_batch, dim=0),
        )
    return collate_fn
...
def evaluate(model, char_to_idx, idx_to_char, group_srcs, group_gt_sets, device, beam_size=8):
    ...
    for i in range(n):
        ...
        if i < 3:
            print(f"\n[Eval {i+1}] Regex: {src[:80]}...")
            print(f"  GT({len(gt_set)}): {list(gt_set)[:3]}")
            print(f"  Greedy: {pred}")
            print(f"  Beam: {beams[:3]}")
    
    metrics = {
        "greedy_exact": greedy_hit / n,
        "greedy_startswith": greedy_starts / n,
        f"beam@{beam_size}_hit": beam_hit / n,
    }
...
def build_onecycle_scheduler(
    ...
    print(f"[Scheduler] OneCycleLR: steps_per_epoch={steps_per_epoch}, "
          f"total_steps={total_steps}, max_lr={max_lr}")
...
def train_one_epoch(
    ...
    loader: DataLoader,
    optimizer: optim.Optimizer,
    scheduler: Optional[optim.lr_scheduler.OneCycleLR],
    device: torch.device,
    pad_idx: int,
    grad_clip: float,
    ...
    for src, trg, trg_weight, sample_weight in tqdm(loader, desc="Training", leave=False):
        src = src.to(device)
        trg = trg.to(device)
        ...
        with amp.autocast(enabled=scaler is not None):
            ...
            # 应用 sample 权重到每个 token
            sw_expand = sample_weight.unsqueeze(1).expand(B, Tm1).reshape(-1)
        ...
        total_loss += loss.item() * max(1, accumulation_steps)
        n_batch += 1
    ...
    return total_loss / max(1, n_batch), global_steps
...
def parse_args():
    p = argparse.ArgumentParser(description="Train Regex->Prefix model with dynamic data generation (V2)")
    ...
    # 样本权重参数
    p.add_argument("--weight_mode", type=str, default="expose",
                   choices=["uniform", "expose", "prefix", "combined"],
                   help="Weight mode: uniform, expose (harder=higher), prefix (longer=higher), combined")
    p.add_argument("--expose_alpha", type=float, default=0.5,
                   help="Exponent for expose ratio weighting (lower expose -> higher weight)")
    p.add_argument("--prefix_alpha", type=float, default=0.0,
                   help="Exponent for prefix length weighting")
    p.add_argument("--tb_beta", type=float, default=0.0,
                   help="Exponent for trgm_budget weighting (from stats field)")
    p.add_argument("--ml_beta", type=float, default=0.0,
                   help="Exponent for max_literal_run_len weighting (from stats field)")
    ...
    # 学习率调度
    p.add_argument("--use_onecycle", action="store_true", default=True,
    ...
    p.add_argument("--save_dir", type=str, default="./models/regex2prefix_v2")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("Configuration (V2 with weights, scheduler, accumulation):")
    ...
    data_scheduler = DynamicDataScheduler(
        ...
        expose_alpha=args.expose_alpha,
    ...
    if args.use_onecycle:
        # 估算每个 epoch 的步数
        estimated_samples = min(samples_per_epoch, len(records))
        steps_per_epoch = math.ceil(estimated_samples / args.batch_size / max(1, args.accumulation_steps))
        
        lr_scheduler = build_onecycle_scheduler(
            optimizer=optimizer,
    ...
    print(f"  LR scheduler: {'OneCycleLR' if args.use_onecycle else 'None'}")
    print("=" * 70)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # 动态生成数据（带权重）
        src_list, trg_list, weight_list = data_scheduler.get_epoch_data(epoch)
        
        dataset = Regex2PrefixDataset(
            src_list, trg_list, char_to_idx, idx_to_char,
        ...
        # 训练
        loss, epoch_steps = train_one_epoch(
            model=model,
            loader=loader,
            optimizer=optimizer,
        ...
        current_lr = lr_scheduler.get_last_lr()[0] if lr_scheduler else args.lr
        print(f"[Epoch {epoch}] Loss: {loss:.4f}, LR: {current_lr:.2e}, "
              f"Steps: {epoch_steps}, Total: {total_steps}, Time: {time.time()-t0:.1f}s")
        
        # 评估
        if epoch % args.eval_interval == 0:
            metrics = evaluate(
                model, char_to_idx, idx_to_char,
                group_srcs, group_gt_sets, device, args.beam_size
            )
            
            current_metric = metrics[f"beam@{args.beam_size}_hit"]
