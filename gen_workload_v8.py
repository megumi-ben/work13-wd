...
def gap_pattern(N: int, kind: GapKind) -> str:
    """
    Gap families for workload diversity / fairness control.
      - anychar: (?:.|\\n){0,N}?
      - space  : (?:[[:space:]]){0,N}?   (POSIX space class; includes newline)
      - nonword: (?:[^A-Za-z0-9_]){0,N}? (roughly \\W; includes space/punct/newline)
    """
    N = max(0, int(N))
    if kind == "space":
        return rf"(?:[[:space:]]){{0,{N}}}?"
...
def _render_structured_token(
    tok: str,
    *,
    digit_mode: str,
    rnd: random.Random,
    generalize_prob: float,
) -> str:
    """
    digit_mode:
      - "generalize": always generalize digit runs to [0-9]{k}
      - "literal"   : keep token literal (escaped)
      - "mixed"     : generalize w.p. generalize_prob, else literal
    """
    if not tok:
...
def _render_repeat(cls: str, lo: int, hi: int) -> str:
    """
    Render quantifier for `cls` with range [lo, hi].
    - {0,0} => ""   (omit entirely)
    - {1,1} => cls  (omit quantifier)
    - {k,k} => cls{k}
    - {lo,hi} => cls{lo,hi}
    """
    lo = int(lo)
...
def _anchor_alt(gram: str, n: int, *, alt_mode: str) -> str:
    """
    alt_mode:
      - "light": (?:abc|ab[WORD_CH]) / (?:ab|a[WORD_CH])
      - "off"  : literal gram
    """
    gram = gram or ""
...
def pick_islands_by_exposure_budget(
    ...
    *,
    ...
    for i, tok in enumerate(tokens):
        ...
        pat = _render_structured_token(
            tok,
            digit_mode=struct_digit_mode,
            rnd=rnd,
            generalize_prob=struct_generalize_prob,
        )
    ...
    # old behavior (default): structured first, then ngrams
    ...
    if not enforce_mix:
        ...
        # mix behavior: enforce approx structured fraction among selected islands
        struct_positions = list(struct_cand.keys())
        ngram_positions = list(ngram_cand.keys())
        ...
        while (struct_positions or ngram_positions) and len(chosen) < k_max and exposure_sum < budget:
            ...
            if need_struct and need_ngram:
                ...
                # quotas filled; pick remaining by top-score
    ...
    # ensure at least one ngram unless user enforces struct_ratio==1.0
    ...
    meta = {
        ...
        "struct_ratio_target": float(struct_ratio),
        "struct_ratio_actual": float(sum(1 for x in islands if x.kind == "struct")) / float(len(islands)) if islands else 0.0,
        "struct_digit_mode": str(struct_digit_mode),
        "struct_generalize_prob": float(struct_generalize_prob),
        "ngram_alt_mode": str(ngram_alt_mode),
    }
    return islands, meta
...
def assemble_regex_from_islands(
    ...
    *,
    ...
    if case_insensitive:
        parts.append("(?i)")
    if prefix_any_g > 0:
        parts.append(gap_pattern(prefix_any_g, gk))
    ...
    if suffix_any_g > 0:
        parts.append(gap_pattern(suffix_any_g, gk))
...
def main() -> None:
    ...
    ap.add_argument("--pg_dsn", default="dbname=sql_data_test user=postgres host=localhost port=5432")
    ...
    ap.add_argument("--tmp_dir", default="/home/lyz/wd/LLM_RegularSearch/v2_data/workload/tmp_workers", help="Default: <out_dir>/tmp_workers")
    ...
    ap.add_argument(
        ...
        type=float,
        required=True,
        help="Exposure budget: exposed_len(word-chars) / total_word_chars(seed).",
    )
    ...
    ap.add_argument("--gap_kind", choices=["anychar", "space", "nonword"], default="anychar")
    ...
    # new realism / fairness knobs (optional; defaults preserve old behavior)
    ap.add_argument(
        "--struct_ratio",
        type=float,
        default=-1.0,
        help="If in [0,1], enforce approx fraction of structured islands among selected islands. Default -1 keeps old behavior (structured-first).",
    )
    ap.add_argument("--struct_digit_mode", choices=["generalize", "literal", "mixed"], default="generalize")
    ap.add_argument("--struct_generalize_prob", type=float, default=0.7, help="Used only when struct_digit_mode=mixed.")
    ap.add_argument("--ngram_alt_mode", choices=["light", "off"], default="light")
        ...
        for wid, n_target in enumerate(targets):
            ...
            futures.append(
                ex.submit(
                    ...
                    # new knobs
                    args.struct_ratio,
                    args.struct_digit_mode,
                    args.struct_generalize_prob,
                    args.ngram_alt_mode,
                    # hygiene
    ...
    print("[INFO] policies: --struct_ratio (or default structured-first), --struct_digit_mode, --ngram_alt_mode, --gap_kind.")
